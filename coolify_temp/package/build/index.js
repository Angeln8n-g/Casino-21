    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            if (!this.axiosInstance) {
                throw new McpError(ErrorCode.InvalidRequest, 'Coolify not configured. Set COOLIFY_BASE_URL and COOLIFY_TOKEN environment variables.');
            }
            ...
            try {
                switch (request.params.name) {
                    ...
                    case 'update_server': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: suuid, ...serverPatch } = args;
                    ...
                    case 'update_project': {
                        if (!args.project_uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'project_uuid is required');
                        const { project_uuid: puuid, ...projectPatch } = args;
                    ...
                    case 'get_environment': {
                        if (!args.project_uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'project_uuid is required');
                        if (!args.environment_name_or_uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'environment_name_or_uuid is required');
                    ...
                    case 'create_environment': {
                        if (!args.project_uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'project_uuid is required');
                        const { project_uuid: epuuid, ...envBody } = args;
                    ...
                    case 'update_application': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: auuid, ...appPatch } = args;
                    ...
                    case 'delete_application': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: dauuid, ...deleteParams } = args;
                    ...
                    case 'start_application': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: sauuid, ...startParams } = args;
                    ...
                    case 'stop_application': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: stauuid, ...stopParams } = args;
                    ...
                    case 'create_application_env': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: aeuuid, ...envBody } = args;
                    ...
                    case 'update_application_env': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: uaeuuid, ...envPatch } = args;
                    ...
                    case 'delete_application_env': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        if (!args.env_uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'env_uuid is required');
                    ...
                    case 'create_application_storage': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: astuuid, ...storageBody } = args;
                    ...
                    case 'delete_application_storage': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        if (!args.storage_uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'storage_uuid is required');
                    ...
                    case 'create_application_scheduled_task': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: atsuuid, ...taskBody } = args;
                    ...
                    case 'delete_application_scheduled_task': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        if (!args.task_uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'task_uuid is required');
                    ...
                    case 'update_database': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: dbuuid, ...dbPatch } = args;
                    ...
                    case 'delete_database': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: deldbuuid, ...dbDeleteParams } = args;
                    ...
                    case 'stop_database': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: stopdbuuid, ...stopDbParams } = args;
                    ...
                    case 'create_database_backup': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: bkuuid, ...backupBody } = args;
                    ...
                    case 'list_database_backup_executions': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        if (!args.scheduled_backup_uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'scheduled_backup_uuid is required');
                    ...
                    case 'create_database_env': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: dbenvuuid, ...dbEnvBody } = args;
                    ...
                    case 'update_service': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: svuuid, ...svPatch } = args;
                    ...
                    case 'delete_service': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: delsvuuid, ...svDeleteParams } = args;
                    ...
                    case 'stop_service': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: stsvuuid, ...stopSvParams } = args;
                    ...
                    case 'restart_service': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: rsvuuid, ...restartSvParams } = args;
                    ...
                    case 'create_service_env': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: svenvuuid, ...svEnvBody } = args;
                    ...
                    case 'create_service_scheduled_task': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: ststuuid, ...stBody } = args;
                    ...
                    case 'list_deployments_by_application': {
                        if (!args.uuid)
                            throw new McpError(ErrorCode.InvalidParams, 'uuid is required');
                        const { uuid: appDeployUuid, ...deployParams } = args;
                    ...
                    case 'delete_github_app': {
                        if (args.github_app_id === undefined)
                            throw new McpError(ErrorCode.InvalidParams, 'github_app_id is required');
                ...
                if (axios.isAxiosError(error)) {
                    throw new McpError(ErrorCode.InternalError, `Coolify API error: ${error.response?.data?.message || error.response?.data?.error || error.message}`);
                }
                throw error;
            }
        });
