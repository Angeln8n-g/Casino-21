...
export type ActionType = 'llevar' | 'formar' | 'formarPar' | 'cantar' | 'colocar' | 'aumentarFormacion' | 'botar';
...
export interface BotarAction extends BaseAction {
  type: 'botar';
  cardId: string;
}
...
export type Action = LlevarAction | FormarAction | FormarParAction | CantarAction | ColocarAction | AumentarFormacionAction | BotarAction;
...
export class DefaultActionValidator implements ActionValidator {
    ...
    const backtrack = (startIndex: number, currentSum: number, groupsFormed: number): boolean => {
      ...
      for (let i = startIndex; i < numbers.length; i++) {
          ...
          if (!used[i] && currentSum + num <= target) {
            used[i] = true;
            if (backtrack(i + 1, currentSum + num, groupsFormed)) return true;
            used[i] = false;
          }
        }
      return false;
  ...
  private validateColocar(state: GameState, action: ColocarAction | BotarAction): ValidationResult {
    const player = state.players.find(p => p.id === action.playerId)!;
