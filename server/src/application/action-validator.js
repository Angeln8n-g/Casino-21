...
exports.DefaultActionValidator = void 0;
...
class DefaultActionValidator {
    validate(state, action) {
        ...
        if (!currentPlayer || currentPlayer.id !== action.playerId) {
            return { isValid: false, error: types_1.ErrorCode.NOT_YOUR_TURN };
        }
        const player = state.players.find(p => p.id === action.playerId);
        if (!player) {
            return { isValid: false, error: types_1.ErrorCode.INVALID_STATE };
        }
        if (action.type !== 'cantar') { // cantar uses cardId too
            const handCard = player.hand.find(c => c.id === action.cardId);
            if (!handCard) {
                return { isValid: false, error: types_1.ErrorCode.CARD_NOT_IN_HAND };
            }
        }
        ...
        switch (action.type) {
            ...
            case 'colocar':
            case 'botar': // Para el sistema interno, botar es igual a colocar en términos de validación de reglas básicas (dejar la carta en la mesa)
                return this.validateColocar(state, action);
            default:
                return { isValid: false, error: types_1.ErrorCode.INVALID_ACTION };
        }
    }
    validateLlevar(state, action) {
        const player = state.players.find(p => p.id === action.playerId);
        const handCard = player.hand.find(c => c.id === action.cardId);
        ...
        for (const formationId of action.formationIds) {
            ...
            if (!formation) {
                return { isValid: false, error: types_1.ErrorCode.FORMATION_NOT_FOUND };
            }
        ...
        for (const boardCardId of action.boardCardIds) {
            ...
            if (isCanted && handCard.rank !== 'A') {
                return { isValid: false, error: types_1.ErrorCode.CARD_PROTECTED };
            }
        }
    ...
    canPartitionIntoSum(numbers, target) {
        if (numbers.length === 0)
            return true;
        ...
        const backtrack = (startIndex, currentSum, groupsFormed) => {
            if (groupsFormed === sum / target)
                return true;
            ...
            for (let i = startIndex; i < numbers.length; i++) {
                ...
                if (num === undefined)
                    continue;
                if (!used[i] && currentSum + num <= target) {
                    used[i] = true;
                    if (backtrack(i + 1, currentSum + num, groupsFormed))
                        return true;
                    used[i] = false;
                }
            }
    ...
    validateFormar(state, action) {
        ...
        if (sum > 14) {
            return { isValid: false, error: types_1.ErrorCode.INVALID_FORMATION_SUM };
        }
    ...
    validateFormarPar(state, action) {
        const player = state.players.find(p => p.id === action.playerId);
        const handCard = player.hand.find(c => c.id === action.cardId);
    ...
    hasTakeCardForPendingFormations(state, playerId, hand, usedCardId) {
        ...
        return pendingFormations.every(formation => remainingHand.some(c => c.value === formation.value || (c.rank === 'A' && formation.value === 14)));
    }
    validateAumentarFormacion(state, action) {
        const player = state.players.find(p => p.id === action.playerId);
        const handCard = player.hand.find(c => c.id === action.cardId);
        const formation = state.board.formations.find(f => f.id === action.formationId);
        if (!formation) {
    ...
    validateColocar(state, action) {
        ...
        const hasPendingCanted = state.board.cantedCards.some(c => c.playerId === player.id);
        if (hasPendingCanted) {
...
exports.DefaultActionValidator = DefaultActionValidator;
