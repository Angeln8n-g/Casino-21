# "Cómo Jugar" Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a public "Cómo Jugar" (How to Play) page at route `/como-jugar` explaining Casino 21 rules and Kasino21 platform features.

**Architecture:** New component `src/web/components/ComoJugar.tsx` following the exact pattern of `PrivacyPolicy.tsx` (named export, SEO via useEffect, sticky header, hero section, max-w-4xl content areas, LegalFooter). Registered as a lazy-loaded route in `App.tsx` via `window.location.pathname` check.

**Tech Stack:** React 19 + TypeScript + Tailwind CSS (no routing library, pathname-based routing)

---

### Task 1: Create the "Cómo Jugar" page component

**Files:**
- Create: `src/web/components/ComoJugar.tsx`

- [x] **Step 1: Create the file**

Write `src/web/components/ComoJugar.tsx` with the full page content following the exact pattern of `PrivacyPolicy.tsx`:
...
NOTE: The `Section`, `ActionCard`, `FeatureRow`, and `LegalFooter` sub-components are identical in pattern to the ones in `PrivacyPolicy.tsx` (minus the icon prop on LegalFooter, using `string` instead of a union to keep things simple).
...
- [x] **Step 1: Add the lazy import**

After the existing lazy imports (around line 16), add:

```tsx
const ComoJugar = lazy(() => import('./components/ComoJugar').then(m => ({ default: m.ComoJugar })));
```

- [x] **Step 2: Add the route check**

In the `App` function, after the existing legal page route checks (after the `/cookies` check), add:
...
Run `npm run build` to confirm both frontend and backend compile without errors.

### Self-Review Checklist

1. **Spec coverage:** Every section from the design spec is covered in Task 1 (Objetivo, Valor, Mecánicas, Puntuación, Victoria, Kasino21 features). The route registration is covered in Task 2. ✓
2. **Placeholder scan:** No TBD, TODO, or incomplete sections. ✓
3. **Type consistency:** `LegalFooter` uses `string` for `current` prop (not the union type since ComoJugar has its own copy). The rest is straightforward React. ✓
